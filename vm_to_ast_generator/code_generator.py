...
class CodeGenerator(NodeVisitor):
    ...
    def visit_Assign(self, node):
        ...
        for token in str(expr).split(' '):
            if token.isdigit():
                vm_code.append(f'push constant {token}')
            elif token in binary_operators.keys():
                vm_code.append(f'call {binary_operators[token]} 2')
            elif token.isalnum():
                if token not in self.symbol_table.keys():
                    self.symbol_table[token] = self.last_addr
                    self.last_addr += 1
                vm_code.append(f"push local {self.symbol_table[token]}")

        # LHS check for symbol in symbol table
        if variable_name not in self.symbol_table.keys(): #if not in symbol table
            self.symbol_table[variable_name] = self.last_addr
            self.last_addr += 1
    ...
    def visit_CompoundStatement(self, node):
        ...
        2. Tokenize the postfix form and scan from left to right. 
            ...
            2c. If you encounter a binary operator, generate VM command to perform operation.
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print('Usage: Python code_generator.py file-name.txt file-name.vm')
        ...
        with open(output_file, "w") as f:
            f.write(f"function {file_name} {len(code_generator.symbol_table)}\n")
            for i in vm_code:
                f.write(f"{i}\n")
            f.write(f"return")
